...
// create table with columnType columns
func createTable(db *sql.DB, schema string, tableName string, columns []string, columnTypes []string) (*sql.Stmt, error) {
	for i, col := range columns {
		columnTypes[i] = fmt.Sprintf("%s %s", col, columnTypes[i])
	}
